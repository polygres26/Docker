#!/usr/bin/env bash
# Runs sayonora-warp's Main. Needs the --add-opens flags below on JDK 17+ (confirmed required on
...
# a plain `java -jar target/sayonora-warp.jar` run with the cache off (the default) works fine
