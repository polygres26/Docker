...
public final class DynamoWireServer {
    ...
    public DynamoWireServer(int port, String pgHost, int pgPort, String pgDatabase, String pgUser, String pgPassword,
            com.sayonora.warp.cluster.RowCache cache) {
        this(port, pgHost, pgPort, pgDatabase, pgUser, pgPassword, cache, com.sayonora.warp.acl.ConnectionGate.DISABLED);
    }

    public DynamoWireServer(int port, String pgHost, int pgPort, String pgDatabase, String pgUser, String pgPassword,
            com.sayonora.warp.cluster.RowCache cache, com.sayonora.warp.acl.ConnectionGate connectionGate) {
        this(port, pgHost, pgPort, pgDatabase, pgUser, pgPassword, cache, connectionGate,
                ...
                com.sayonora.warp.dynamowire.auth.AwsIamCredentialStore.DISABLED);
    }
     ...
     */
    public DynamoWireServer(int port, com.sayonora.warp.core.BackendRegistry backendRegistry, com.sayonora.warp.cluster.RowCache cache,
    ...
    private DynamoWireServer(int port, PgItemStore store, com.sayonora.warp.cluster.RowCache cache,
            com.sayonora.warp.acl.ConnectionGate connectionGate,
            com.sayonora.warp.http.auth.AccessContextResolver oauth,
            com.sayonora.warp.dynamowire.auth.AwsIamCredentialStore awsIamCredentials,
            com.sayonora.warp.core.SqlMetricsCollector sqlMetrics) {
        ...
        com.sayonora.warp.dynamowire.auth.SigV4Verifier sigV4Verifier =
                new com.sayonora.warp.dynamowire.auth.SigV4Verifier(awsIamCredentials);
        this.server = new Server(port);
        // A/B routing (com.sayonora.warp.ab): the same authentication as below, run before a request leaves for the cloud
        com.sayonora.warp.ab.AbRouting.Authenticator abAuth = (request, response, bodyBytes) -> {
            if (!connectionGate.acceptHttp(request)) {
                return com.sayonora.warp.ab.AbRouting.AuthResult.denied(403, "AccessDeniedException", "forbidden");
            }
            String b = bodyBytes == null ? "" : new String(bodyBytes, StandardCharsets.UTF_8);
            if (awsIamCredentials.isEnabled()) {
                com.sayonora.warp.dynamowire.auth.SigV4Verifier.Result r = sigV4Verifier.verify(request, b);
                return r.valid() ? com.sayonora.warp.ab.AbRouting.AuthResult.ok(r.accessKeyId())
                        : com.sayonora.warp.ab.AbRouting.AuthResult.denied(401, "UnrecognizedClientException", r.reason());
            }
            return oauth.enforce(request, response) == null ? com.sayonora.warp.ab.AbRouting.AuthResult.respondedAlready()
                    : com.sayonora.warp.ab.AbRouting.AuthResult.ok(null);
        };
        server.setHandler(com.sayonora.warp.ab.AbRouting.wrap("dynamodb", new AbstractHandler() {
            @Override
            public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
                ...
                if (awsIamCredentials.isEnabled()) {
                    com.sayonora.warp.dynamowire.auth.SigV4Verifier.Result sigResult = sigV4Verifier.verify(request, body);
                    if (!sigResult.valid()) {
