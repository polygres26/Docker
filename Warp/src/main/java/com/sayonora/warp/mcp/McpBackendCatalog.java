...
final class McpBackendCatalog {
    ...
    private boolean storeRunning(BackendKind k) {
        return switch (k) {
            ...
            case SQS, OPENSEARCH, NEO4J, S3STORE, REDIS, AZBLOB, AZQUEUE, AZTABLE, SNS, KINESIS, AWSPARAMS, GCS, BIGTABLE, PUBSUB, FIRESTORE, DATASTORE, CQLSTORE, KAFKASTORE, GREMLINSTORE, COSMOSSTORE, AMQP -> true; // described straight from the hosting Postgres
            default -> false;
        };
    }
    ...
    static BackendKind kindOf(StoreType st) {
        return switch (st) {
            ...
            case CQL -> BackendKind.CQLSTORE;
            case KAFKA -> BackendKind.KAFKASTORE;
            case GREMLIN -> BackendKind.GREMLINSTORE;
            case COSMOS -> BackendKind.COSMOSSTORE;
            case AMQP -> BackendKind.AMQP;
        };
    }
}
