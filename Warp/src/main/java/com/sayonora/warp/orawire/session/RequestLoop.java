...
import com.sayonora.warp.config.FailedStatementLog;
...
import com.sayonora.warp.core.StatementPipeline;
import com.sayonora.warp.core.UntranslatableQueryException;
import com.sayonora.warp.orawire.translator.BindVariableRewriter;
import com.sayonora.warp.xa.XaTransaction;
import com.sayonora.warp.orawire.translator.DualTableRewriter;
import com.sayonora.warp.orawire.ttc.BindParam;
import com.sayonora.warp.orawire.ttc.ColumnMetadata;
import com.sayonora.warp.orawire.ttc.ExecuteRequest;
import com.sayonora.warp.orawire.ttc.ExecuteRequestReader;
import com.sayonora.warp.orawire.ttc.FetchRequest;
import com.sayonora.warp.orawire.ttc.OracleProcedureCatalog;
import com.sayonora.warp.orawire.ttc.ResponseWriter;
import com.sayonora.warp.orawire.ttc.TtcConstants;
...
public final class RequestLoop {
    ...
    private final com.sayonora.warp.core.LazyPooledConnection pgConnection;
    private final com.sayonora.warp.core.LazyPooledConnection oracleConnection;
    ...
    private com.sayonora.warp.orawire.backend.NativeOracleExecutor nativeExecutor;
    ...
    // Shim/pg_oracle's unqualified V$/DBA_*/DBMS_*/UTL_* names resolve at all -- confirmed live
    ...
    private final JdbcBackendExecutor terminalExecutor =
            new JdbcBackendExecutor(null, new com.sayonora.warp.core.access.OraclePgEmulationSessionInitializer());
    private final StatementPipeline reusablePipeline;
    private final com.sayonora.warp.core.AccessContext accessContext;
    // Connect-time backend routing (the TNS service name / login user); see core/ConnectionRouter.
    private com.sayonora.warp.core.ConnectionRoute route = com.sayonora.warp.core.ConnectionRoute.UNROUTED;

    public void setConnectionRoute(com.sayonora.warp.core.ConnectionRoute route) {
        this.route = route == null ? com.sayonora.warp.core.ConnectionRoute.UNROUTED : route;
    }

    private record StatementSignature(String sql, int[] bindTypes) {
    }

    public RequestLoop(TnsPacketReader reader, OutputStream out, com.sayonora.warp.core.LazyPooledConnection pgConnection,
            com.sayonora.warp.core.LazyPooledConnection oracleConnection, List<Connection> replicaConnections,
        ...
        this(reader, out, pgConnection, oracleConnection, replicaConnections, xaTransaction, options, sharedStages,
                backendRegistry, null, null, com.sayonora.warp.core.AccessContext.ANONYMOUS);
    }
    ...
    public RequestLoop(TnsPacketReader reader, OutputStream out, com.sayonora.warp.core.LazyPooledConnection pgConnection,
        ...
        this(reader, out, pgConnection, oracleConnection, replicaConnections, xaTransaction, options, sharedStages,
                backendRegistry, oracleUsername, oraclePassword, com.sayonora.warp.core.AccessContext.ANONYMOUS);
    }
    ...
    public RequestLoop(TnsPacketReader reader, OutputStream out, com.sayonora.warp.core.LazyPooledConnection pgConnection,
            ...
            XaTransaction xaTransaction,
            ServerOptions options, List<PipelineStage> sharedStages, com.sayonora.warp.core.BackendRegistry backendRegistry,
            String oracleUsername, String oraclePassword, com.sayonora.warp.core.AccessContext accessContext) {
        ...
        this.reusablePipeline = new StatementPipeline(sharedStages,
                new com.sayonora.warp.core.RoutingBackendExecutor(backendRegistry, terminalExecutor,
                new com.sayonora.warp.xa.XaRecoveryLog(options),
    ...
    private void handleExecute(ExecuteRequest request, TtcWriter w, int callNumber) throws SQLException {
        ...
        if (canMultiplex) {
            ...
            if (com.sayonora.warp.core.SessionStatePins.sessionStateReason(SourceDialect.ORACLE, request.sqlText) != null) {
        ...
        if (result.isQuery()) {
            ...
            // Real row count included either way. When this batch also exhausts the cursor
            // (fewer rows existed than request.numIters asked for), the trailing status must
            // ALSO carry an inline ORA-01403 warning alongside that real rowcount --
            // writeSuccessEndWithWarning, not a plain writeSuccessEnd -- confirmed against a
            // real Oracle packet capture (ScratchRealOracleCaptureTest/ScratchDecodeRealOracleBytesTest):
            ...
            // adding the SAME trailing warning fields real Oracle sends alongside them. Genuine
            // exhaustion -- a FETCH call made after the cursor is already empty, no new rows at
            // all -- is unaffected and still correctly signaled via writeErrorEnd elsewhere.
            if (usedNativeOciExecuteFallback) {
                ...
                long written = writeRows(w, request.numIters);
                boolean cursorExhausted = fetchPosition >= openRows.size();
                if (cursorExhausted) {
                    ResponseWriter.writeSuccessEndWithWarning(
                            w, written, openCursorId, 1403, "ORA-01403: no data found\n", callNumber);
                } else {
                    ResponseWriter.writeSuccessEnd(w, written, openCursorId, callNumber);
                }
            }
        } else {
    ...
    private void handleExecuteNative(ExecuteRequest request, TtcWriter w) throws SQLException {
        if (nativeExecutor == null) {
            nativeExecutor = new com.sayonora.warp.orawire.backend.NativeOracleExecutor(
                    options, oracleUsername, oraclePassword);
        }
        com.sayonora.warp.orawire.backend.NativeOracleExecutor.NativeQueryResult result =
