...
public final class DialectTranslations {
    ...
    private static final Map<SourceDialect, Function<String, String>> NORMALIZERS = Map.of(
            ...
            SourceDialect.WARP_NATIVE, DialectTranslations::renderIdentity,
            // MCP's execute_sql tool sends plain SQL already in Postgres dialect, same as gRPC's
            // own native protocol (WARP_NATIVE) -- needs the same no-op identity normalizer,
    ...
    // Oracle's DATE always carries a time-of-day component to the second -- unlike Postgres's own
    // DATE, which is date-only -- so a column declared DATE in Oracle can and does store times;
    // leaving it as Postgres DATE would silently truncate that on every write. Matches pg_oracle's
    // own TO_DATE() fidelity fix (Shim/pg_oracle/sql/pg_oracle--0.1.sql) -- same underlying gap,
    // fixed on both the DDL-type side here and the function-return-type side there. `\bDATE\b`
    // doesn't false-positive on TO_DATE/SYSDATE/etc. (no word-boundary between the preceding
    // underscore-or-letter and D), and a `DATE 'literal'` ANSI date-literal prefix maps cleanly
    // to the equally-valid `TIMESTAMP 'literal'` form, not just the column-type position.
    private static final Pattern DATE_TYPE = Pattern.compile("(?i)\\bDATE\\b");
    ...
    private static String mapOracleDdlTypes(String sql) {
        ...
        out = SqlLiterals.replaceOutsideLiterals(out, DATE_TYPE, m -> "TIMESTAMP");
        return out;
    }
    ...
    // Schema-qualifying TO_CHAR/TO_DATE unconditionally, not just leaving them bare for
    // db_emulation's search_path to resolve -- a real bug found live, more serious than the
    // already-documented "two-argument form doesn't get RR/bare-FF/X translation" formatting
    // gap: pg_catalog's own to_date(text, text) returns Postgres's date type (no time-of-day at
    // all), and a bare, unqualified two-argument TO_DATE(...) call always resolves to THAT one
    // over pg_oracle's own oracle_catalog.to_date(text, text) (which returns timestamp
    // specifically to preserve Oracle DATE's time component, see Shim/pg_oracle's own comment on
    // that function) -- Postgres resolves an exact-argument-type tie via schema search order,
    // and pg_catalog is always searched first regardless of this extension's search_path
    // additions. Confirmed live: `INSERT INTO t VALUES (1, TO_DATE('2026-06-15 09:45:30',
    // 'YYYY-MM-DD HH24:MI:SS'))` silently stored `2026-06-15 00:00:00` -- the time component was
    // gone, not just displayed oddly. A negative lookbehind for a preceding "." avoids
    // double-qualifying a call that's already schema-qualified.
    private static final Pattern TO_CHAR_CALL = Pattern.compile("(?i)(?<!\\.)\\bTO_CHAR\\s*\\(");
    private static final Pattern TO_DATE_CALL = Pattern.compile("(?i)(?<!\\.)\\bTO_DATE\\s*\\(");
    ...
    private static String normalizeOracle(String sql) {
        ...
        // Shim/pg_oracle/test/orawire-integration/README.md's Bug 4), but a working, degraded
    ...
    // MySQL's introspection commands -- see Shim/pg_mysql/sql/pg_mysql--0.1.sql's own header comment
