...
/**
 * The missing link between orawire and Shim/pg_oracle: every orawire session's backend Postgres
 ...
 * while building pg_oracle (see Shim/pg_oracle/README.md) that without this, orawire's translated SQL
 ...
 * (Shim/pg_oracle's dbms_session.set_context('warp_ctx', ...) plus set_identifier() for the
 * userId) -- not because orawire needs it for anything today, but because it's the exact,
 * intended integration point Shim/pg_oracle's VPD section was designed for: an Oracle-migrated
 ...
 * pg_oracle extension), {@link com.sayonora.warp.core.PgOracleSupport} detects that up front and
 ...
 * to -- see that class's own javadoc, and {@link com.sayonora.warp.core.DialectTranslations} for
 ...
 *
 * A real, subtle interaction with {@link com.sayonora.warp.core.LazyPooledConnection} found
 * live and fixed on pg_oracle's own side (Shim/pg_oracle's db_emulation_assign_hook, see that
...
public final class OraclePgEmulationSessionInitializer implements NativeRlsSessionInitializer {
    ...
    public void initialize(Connection connection, AccessContext accessContext) throws SQLException {
        ...
        // there (see this class's own now-outdated javadoc, and com.sayonora.warp.core.
        ...
        // caching is safe.
        if (!com.sayonora.warp.core.PgOracleSupport.isAvailable(connection)) {
            return;
        }
        ...
        // (see Shim/pg_oracle/README.md's DBMS_NETWORK_ACL_ADMIN-pattern privilege model), so the
