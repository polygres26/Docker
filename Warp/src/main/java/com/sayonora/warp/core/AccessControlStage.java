...
import com.sayonora.warp.core.access.ColumnMasker;
import com.sayonora.warp.core.access.SqlTableReferences;
import com.sayonora.warp.core.access.WhereClauseInjector;
...
public final class AccessControlStage implements PipelineStage {
    ...
    public AccessControlStage(AccessPolicy policy, com.sayonora.warp.audit.AuditLog auditLog) {
    ...
    private Statement enforce(Statement statement, AccessPolicy policy) throws SQLException {
        ...
        for (AccessPolicy.RowFilter rule : policy.rowFilters()) {
            ...
            if (value == null) {
                ...
                audit(com.sayonora.warp.audit.AuditEvent.Type.ACCESS_DENIED, accessContext.userId(),
            ...
            audit(com.sayonora.warp.audit.AuditEvent.Type.ROW_FILTER_APPLIED, accessContext.userId(),
        ...
        for (AccessPolicy.ColumnGrant grant : policy.columnGrants()) {
            ...
            for (String column : grant.columns()) {
                ...
                if (grant.onViolation() == AccessPolicy.OnViolation.MASK) {
                    ...
                    audit(com.sayonora.warp.audit.AuditEvent.Type.COLUMN_MASKED, accessContext.userId(),
    ...
    private void audit(com.sayonora.warp.audit.AuditEvent.Type type, String userId, String summary, java.util.Map<String, String> details) {
        if (auditLog != null) {
            auditLog.record(com.sayonora.warp.audit.AuditEvent.of(type, userId, summary, details));
        }
    }
