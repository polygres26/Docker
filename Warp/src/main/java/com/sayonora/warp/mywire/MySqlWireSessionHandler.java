...
public final class MySqlWireSessionHandler implements Runnable {
    ...
    // See com.sayonora.warp.core.access.MySqlPgEmulationSessionInitializer's own header comment --
    ...
    // GROUP_CONCAT(...), DATE_FORMAT(...), etc. resolve without Shim/pg_mysql needing every caller
    // to schema-qualify mysql_catalog.* by hand.
    private final JdbcBackendExecutor terminalExecutor =
            new JdbcBackendExecutor(null, new com.sayonora.warp.core.access.MySqlPgEmulationSessionInitializer());
    ...
    private com.sayonora.warp.core.BackendRegistry backendRegistry;
    ...
    private void ensurePhysicalTransaction(String sql) throws SQLException {
        ...
        if (lease.pinReason() == null && !lease.hasSettings() && com.sayonora.warp.core.SessionStatePins.isPureRead(SourceDialect.MYSQL, sql)) {
    ...
    private HandshakeStreams performHandshake(DataInputStream in, OutputStream out, MySqlPacket packets) throws IOException {
        ...
        if (backendRegistry != null) {
            route = backendRegistry.connectionRouter().resolve(
                    com.sayonora.warp.core.ConnectionRouter.PROTO_MYSQL, database, username);
            if (route.isRejected()) {
    ...
    private void switchDatabase(OutputStream out, MySqlPacket packets, String database) throws IOException {
        if (backendRegistry != null) {
            com.sayonora.warp.core.ConnectionRoute next = backendRegistry.connectionRouter().resolve(
                    com.sayonora.warp.core.ConnectionRouter.PROTO_MYSQL, database.strip(), loginUser);
            if (next.isRejected()) {
                packets.writePayload(out, MySqlMessages.errPacket(1049, "42000",
    ...
    private static boolean handleSetTimeZone(com.sayonora.warp.core.SessionConnectionLease lease, String sql)
            throws SQLException {
    ...
    private static boolean handleSetIsolationLevel(com.sayonora.warp.core.SessionConnectionLease lease, String sql)
            throws SQLException {
                ...
                if (options.mywireNativeBackend()) {
                    ...
                    if (options.mywireNativeViaDualPort()) {
                        statement = statement.withRouting(statement.workloadClass(),
                                com.sayonora.warp.core.BackendRegistry.MYSQL_NATIVE_DUAL_PORT_NAME);
                    }
                    ...
                    String pinReason = com.sayonora.warp.core.SessionStatePins.sessionStateReason(SourceDialect.MYSQL, sql);
                    if (pinReason != null) {
                    ...
                    try {
                        if (lease.multiplexing() && LAST_INSERT_ID_QUERY.matcher(sql).matches()) {
                            result = ExecutionResult.ofQuery(
                                    List.of(new com.sayonora.warp.core.ColumnInfo("LAST_INSERT_ID()", Types.BIGINT,
                                            20, 0, 20, false, "int8")),
