#!/usr/bin/env python3
"""Start a throwaway Warp with mongowire on free ports and 1 or 2 local Postgres backends (2 = one backend set,
the `mongodb` store sharded across both). Importable (`with Stack(shards=2) as s: s.uri`) or a CLI that prints
{"uri": ...} and stays up until SIGTERM/SIGINT.

  WARP_TEST_PG_LOCAL=1 WARP_TEST_JAR=<jar> python3 mongo_launch_warp.py --state state.json [--shards 2]
"""
import argparse
import json
import os
...
class Stack:
    def __init__(self, shards=1, extra_env=None):
        ...
        try:
            ...
            self.warp = WarpProcess(self.pgs[0], "WARP_MONGOWIRE_PORT", frontend_name="mongowire", extra_env=env)
            self.port = self.warp.frontend_port
            self.uri = f"mongodb://localhost:{self.port}"
    ...
    def _shard(self):
        self.api("PATCH", "/api/backend-sets/default/backends/default", {"enabledStores": ["mongodb"]})
        for i, pg in enumerate(self.pgs[1:], start=2):
            self.api("POST", "/api/backend-sets/default/backends", {
                ...
                "password": "postgres", "enabledStores": ["mongodb"]}, expect=201)
...
def main():
    ...
    json.dump({"uri": stack.uri, "pid": os.getpid(), "warp_pid": stack.warp.process.pid}, open(a.state, "w"))
