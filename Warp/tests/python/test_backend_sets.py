...
def test_existing_config_migrates_into_the_default_set(api):
    ...
    assert {s["id"] for s in data["stores"]} == {"influxdb", "mongodb", "sqs", "neo4j", "opensearch", "dynamodb", "s3", "redis", "azblob", "azqueue", "aztable", "gcs", "bigtable", "pubsub", "firestore", "datastore", "sns", "kinesis", "awsparams", "cql", "kafka"}
