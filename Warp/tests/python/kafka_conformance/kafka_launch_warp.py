"""Starts Warp (WARP_TEST_JAR) on local Postgres process(es) with the kafkawire frontend; 1 or N sharded backends."""
import os
import socket
import sys
...
class KafkaWarp:
    """Warp with the kafka store enabled on `shards` real Postgres servers (or the given `pgs`). Kafka wire protocol on .port;
    extra_env adds Warp environment variables (WARP_POOL_MAX_SIZE, WARP_KAFKAWIRE_AUTH, ...)."""

    def __init__(self, shards=1, extra_env=None, pgs=None, default_store=True):
        self.pgs = pgs or [RealPostgres() for _ in range(shards)]
        env = {**isolated_ports("WARP_KAFKAWIRE_PORT"),
               ...
               "WARP_KAFKAWIRE_SWEEP_MS": "3600000", "WARP_KAFKAWIRE_ADVERTISED_HOST": "localhost", **(extra_env or {})}
        try:
            self.proc = WarpProcess(self.pgs[0], "WARP_KAFKAWIRE_PORT", frontend_name="kafkawire", extra_env=env)
        except BaseException:
            for pg in self.pgs:
                pg.close()
            raise
        self.port = self.proc.frontend_port
        self.frontend_port = self.proc.frontend_port
        self.metrics_port = self.proc.metrics_port
