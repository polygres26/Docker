"""Starts Warp (WARP_TEST_JAR) on local Postgres process(es) with the pubsubwire frontend; 1 or N sharded backends."""
import os
import socket
import sys
...
class PsWarp:
    """Warp with the pubsub store enabled on `shards` real Postgres servers (or the given `pgs`). gRPC on .grpc_port, REST on
    .rest_port; extra_env adds Warp environment variables (WARP_POOL_MAX_SIZE, WARP_PUBSUBWIRE_TOKENS, ...)."""
    ...
    def __init__(self, shards=1, extra_env=None, pgs=None):
        ...
        env = {**isolated_ports("WARP_PUBSUBWIRE_PORT"), "WARP_PUBSUBWIRE_REST_PORT": str(self.rest_port),
               ...
               **(extra_env or {})}
        env["WARP_PUBSUBWIRE_REST_PORT"] = str(self.rest_port) if "WARP_PUBSUBWIRE_REST_PORT" not in (extra_env or {}) \
            else env["WARP_PUBSUBWIRE_REST_PORT"]
        try:
            self.proc = WarpProcess(self.pgs[0], "WARP_PUBSUBWIRE_PORT", frontend_name="pubsubwire", extra_env=env)
