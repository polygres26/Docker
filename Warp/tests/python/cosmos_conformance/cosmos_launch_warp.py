"""Starts Warp (WARP_TEST_JAR) on local Postgres process(es) with the cosmoswire frontend; 1 or N sharded backends."""
import os
import socket
import sys
...
class CosmosWarp:
    """Warp with the cosmos store enabled on `shards` real Postgres servers (or the given `pgs`). Cosmos REST API on .port;
    extra_env adds Warp environment variables (WARP_COSMOSWIRE_KEYS, WARP_COSMOSWIRE_QUERY_PLAN_HANDSHAKE, ...)."""

    def __init__(self, shards=1, extra_env=None, pgs=None, default_store=True):
        self.pgs = pgs or [RealPostgres() for _ in range(shards)]
        env = {**isolated_ports("WARP_COSMOSWIRE_PORT"),
               ...
               "WARP_COSMOSWIRE_SWEEP_SECONDS": "1", **(extra_env or {})}
        try:
            self.proc = WarpProcess(self.pgs[0], "WARP_COSMOSWIRE_PORT", frontend_name="cosmoswire", extra_env=env)
