"""Starts Warp (WARP_TEST_JAR) on local Postgres process(es) with the cqlwire frontend; 1 or N sharded backends."""
import os
import socket
import sys
...
class CqlWarp:
    """Warp with the cql store enabled on `shards` real Postgres servers (or the given `pgs`). CQL native protocol on .port;
    extra_env adds Warp environment variables (WARP_POOL_MAX_SIZE, WARP_CQLWIRE_AUTH, ...)."""

    def __init__(self, shards=1, extra_env=None, pgs=None, default_store=True):
        self.pgs = pgs or [RealPostgres() for _ in range(shards)]
        env = {**isolated_ports("WARP_CQLWIRE_PORT"),
               ...
               "WARP_CQLWIRE_SWEEP_MS": "300", **(extra_env or {})}
        try:
            self.proc = WarpProcess(self.pgs[0], "WARP_CQLWIRE_PORT", frontend_name="cqlwire", extra_env=env)
