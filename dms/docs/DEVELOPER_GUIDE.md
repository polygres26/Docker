# Sayonora DMS — Developer Guide

> **This is a technical/internal reference** for contributors working on Sayonora DMS's codebase
...
```
src/main/java/com/sayonora/dms/
  ...
  llm/       LLM/SLM layer -- explicitly downstream of catalog + workload capture, never
             ...
             LLM configuration page (own rail item, app-wide, not scoped to a connection):
               ...
               - LocalLlamaProcess/LocalLlamaManager (LOCAL): manages llama-server sidecar
                 ...
                 own auto-assigned port starting at SAYONORA_LLM_LOCAL_PORT) rather than a single
                 ...
                 SAYONORA_LLM_LOCAL_SERVER_PATH or PATH; the model file (a .gguf) is set on the
  ...
  report/    FindingsReportGenerator -- the Findings tab's "Download report" PDF, built with
             ...
             earlier visit.
  http/auth/ Single-admin-account session auth (AdminAuth/AuthGuard) -- SAYONORA_ADMIN_USER /
             SAYONORA_ADMIN_PASSWORD, cookie session. Minimal on purpose (no OIDC/multi-user);
...
Admin + saved connections (com.sayonora.dms.core.ConnectionStore, ConnectionRecord):
  - CRUD over a set of named source-database connections, persisted in an embedded HSQLDB file
    at ~/.sayonora/sayonora-store (SAYONORA_DATA_DIR to relocate). Credentials are stored
...
Uploaded reports (com.sayonora.dms.uploads.ReportStore, com.sayonora.dms.llm.ReportAnalyzer):
  ...
  - Report text/metadata persisted the same way connections are (embedded HSQLDB for metadata +
    cached analysis JSON; raw extracted text on disk under ~/.sayonora/reports/<id>.txt, kept out
