 ...
 * {@code NEXAGRES_DMS_WEB_DIR} is set (see {@link #main}), it's wired into a Jetty {@code
...
public class DmsHttpServer extends AbstractHandler {
    ...
    public static void main(String[] args) throws Exception {
        int port = Integer.parseInt(System.getenv().getOrDefault("NEXAGRES_DMS_PORT", "8090"));
        Server server = new Server(port);

        DmsHttpServer api = new DmsHttpServer();
        // NEXAGRES_DMS_WEB_DIR: path to the built advisor/web SPA (its `dist/`). Opt-in --
        ...
        String webDir = System.getenv("NEXAGRES_DMS_WEB_DIR");
        if (webDir != null && !webDir.isBlank() && java.nio.file.Files.isDirectory(java.nio.file.Path.of(webDir))) {
            ...
            if (webDir != null && !webDir.isBlank()) {
                log.warn("NEXAGRES_DMS_WEB_DIR={} is not a directory -- serving API only", webDir);
            }
