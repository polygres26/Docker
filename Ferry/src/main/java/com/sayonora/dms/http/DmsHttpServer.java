...
import com.sayonora.dms.http.auth.AdminAuth;
import com.sayonora.dms.http.auth.AuditGuard;
import com.sayonora.dms.http.auth.AuthGuard;
import com.sayonora.dms.http.auth.LoginRoute;
import com.sayonora.dms.http.auth.LogoutRoute;
import com.sayonora.dms.http.auth.SessionRoute;
import com.sayonora.dms.http.auth.SsoAuth;
import com.sayonora.dms.http.auth.SsoLoginRoute;
 ...
 * {@code SAYONORA_DMS_WEB_DIR} is set (see {@link #main}), it's wired into a Jetty {@code
 ...
 */
public class DmsHttpServer extends AbstractHandler {

    private static final Logger log = LoggerFactory.getLogger(DmsHttpServer.class);
    ...
    public DmsHttpServer() {
        ...
        String ssoSecret = System.getenv("SAYONORA_SSO_JWT_SECRET");
        if (ssoSecret != null && !ssoSecret.isBlank()) {
            try {
                SsoAuth ssoAuth = new SsoAuth(ssoSecret.getBytes(java.nio.charset.StandardCharsets.UTF_8),
                        System.getenv("SAYONORA_SSO_ISSUER"));
                routes.put("/api/sso-login", new SsoLoginRoute(ssoAuth, auth));
            } catch (IllegalStateException e) {
                log.warn("SAYONORA_SSO_JWT_SECRET is set, but {} -- /api/sso-login will not be registered.",
                        e.getMessage());
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        int port = Integer.parseInt(System.getenv().getOrDefault("SAYONORA_DMS_PORT", "8090"));
        Server server = new Server(port);

        DmsHttpServer api = new DmsHttpServer();
        // SAYONORA_DMS_WEB_DIR: path to the built advisor/web SPA (its `dist/`). Opt-in --
        ...
        String webDir = System.getenv("SAYONORA_DMS_WEB_DIR");
        if (webDir != null && !webDir.isBlank() && java.nio.file.Files.isDirectory(java.nio.file.Path.of(webDir))) {
            ...
            if (webDir != null && !webDir.isBlank()) {
                log.warn("SAYONORA_DMS_WEB_DIR={} is not a directory -- serving API only", webDir);
            }
            server.setHandler(api);
        }

        server.start();
        log.info("Sayonora Advisor listening on http://localhost:{}", port);
        server.join();
    }
}
