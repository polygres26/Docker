 ...
 * pg_mysql extension (Shim/pg_mysql) -- db_emulation itself has to live in
 ...
 * pg_sqlserver extension (Shim/pg_sqlserver) -- same "db_emulation lives in
...
db_emulation_assign_hook(int newval, void *extra)
{
	 ...
	 * connection -- see Shim/pg_oracle/README.md): search_path can be
...
/* Exported for Shim/pg_mysql's own SQL script (mysql_catalog.emulation_active())
...
/* Exported for Shim/pg_sqlserver's own SQL script (sys.emulation_active()) --
