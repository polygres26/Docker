...
-- extension collection (see Shim/pg_oracle/README.md for scope and phasing).
--
...
BEGIN
  ...
  IF v_query ~ '\$\d+' THEN
    ...
    plan_line := 'pg_oracle: this sql_id''s text is bind-parameterized ($1, $2, ...) -- '
                 ...
                 || 'Shim/pg_oracle/README.md''s V$SQL_PLAN section.';
    RETURN NEXT;
    RETURN;
  END IF;
...
-- (Warp/.../DialectTranslations.java's TO_CHAR_CALL/TO_DATE_CALL
...
BEGIN
  ...
  -- call itself now (Warp/.../DialectTranslations.java's TO_DATE_CALL
