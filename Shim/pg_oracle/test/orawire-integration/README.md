...
Fixed in `Shim/pg_oracle/src/pg_oracle.c`: the hook now reconciles against the *actual current*
...
```bash
# 1. Postgres with pg_oracle installed (see Shim/pg_oracle/README.md's Building section)
# 2. Build and run Warp pointed at it (see Warp/scripts/run.sh)
