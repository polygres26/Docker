# Warp — Performance: SQL Statistics, RTT, and the Cache-Hit Latency Investigation
...
> Warp's caching gets you, see [`USER_GUIDE.md`](USER_GUIDE.md).
...
2. **Client-observed *and* server-side, always both**: client-observed (Python `time.perf_counter`
   around the call) includes network + client-library overhead (real, but not Warp's to fix);
