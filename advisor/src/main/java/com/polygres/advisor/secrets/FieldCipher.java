 ...
 * {@code POLYGRES_ENCRYPTION_KEY} -- base64, 32 raw bytes; same env var name and format as
 * PolyWire's twin {@code com.polygres.wire.secrets.FieldCipher}, so one key covers both modules).
...
public final class FieldCipher {
    ...
    private static SecretKeySpec keyFromEnv() {
        ...
        if (encoded == null || encoded.isBlank()) {
            if (!warnedOnce) {
                warnedOnce = true;
                log.warn("POLYGRES_ENCRYPTION_KEY is not set -- connection passwords and the "
