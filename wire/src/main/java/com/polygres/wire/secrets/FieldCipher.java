 ...
 * {@code POLYGRES_ENCRYPTION_KEY} -- base64, 32 raw bytes). Applied narrowly, not as a whole-blob
 ...
 * carry the {@code encv1:} prefix unchanged -- rows written before {@code POLYGRES_ENCRYPTION_KEY}
...
public final class FieldCipher {
    ...
    private static SecretKeySpec keyFromEnv() {
        String encoded = System.getenv("POLYGRES_ENCRYPTION_KEY");
        if (encoded == null || encoded.isBlank()) {
            if (!warnedOnce) {
                warnedOnce = true;
                log.warn("POLYGRES_ENCRYPTION_KEY is not set -- backend passwords and AWS IAM "
        ...
        if (raw.length != 32) {
            throw new IllegalStateException("POLYGRES_ENCRYPTION_KEY must decode to exactly 32 bytes "
    ...
    public static String decrypt(String stored) {
        ...
        SecretKeySpec key = keyFromEnv();
        if (key == null) {
            throw new IllegalStateException("field is encrypted (encv1:) but POLYGRES_ENCRYPTION_KEY "
        ...
        try {
            ...
            throw new IllegalStateException("field decryption failed -- wrong POLYGRES_ENCRYPTION_KEY?", e);
        }
    }
}
