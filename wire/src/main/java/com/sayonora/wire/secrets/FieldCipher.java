package com.sayonora.wire.secrets;
 ...
 * {@code SAYONORA_ENCRYPTION_KEY} -- base64, 32 raw bytes). Applied narrowly, not as a whole-blob
 ...
 * carry the {@code encv1:} prefix unchanged -- rows written before {@code SAYONORA_ENCRYPTION_KEY}
...
public final class FieldCipher {
    ...
    private static SecretKeySpec keyFromEnv() {
        ...
        if (encoded == null || encoded.isBlank()) {
            if (!warnedOnce) {
                warnedOnce = true;
                log.warn("SAYONORA_ENCRYPTION_KEY is not set -- backend passwords and AWS IAM "
