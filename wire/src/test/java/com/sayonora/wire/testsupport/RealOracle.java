 ...
 *
 * <p>Ported from {@code migration}'s own test-support package for wire's own real-backend
 * sharding tests. Every URL here uses the double-slash {@code @//host:port/service} form, not the
 * single-slash {@code @host:port/service} form the migration module's original used (both are
 * real, valid Oracle thin-driver syntax) -- {@link com.sayonora.wire.core.TrustedBackendHosts}'s
 * own Oracle URL regex only recognizes the double-slash form (confirmed live: a single-slash
 * shard backend URL was silently rejected as "not trusted" even with its host correctly listed in
 * WARP_TRUSTED_BACKEND_HOSTS, since the host could never be extracted from it in the first place).
 * Worth widening that regex to accept both forms in wire's own main source at some point; not done
 * here to keep this a test-support port, not a product change.
 */
