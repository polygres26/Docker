// This SPA talks directly to NexaGate's own admin API (see
...
// ESTIMATE per node, never an actual post-execution measurement -- this is NexaGate's own
...
// New admin endpoint, built concurrently by a separate agent -- not yet visible in
// MetricsServer.java at the time this client was written. Contract per the spec this page was
// built against: each NexaGate instance heartbeats its identity to the shared config Postgres
