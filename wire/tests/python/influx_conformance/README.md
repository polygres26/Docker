...
| `rtt_bench.py` | single-point write / point-query round trips, for before/after comparisons |
...
rows for 2020 data, the container is OOM-killed), so `corpus.py` bounds every such query to a two-minute window.
